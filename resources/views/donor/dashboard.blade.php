...
@section('content')
    @php
        ...
        $subtitle = $isEn
            ? 'Track your donations, receipts, and supported campaigns in one professional dashboard.'
            : 'تابع تبرعاتك وإيصالاتك والحملات التي دعمتها من خلال لوحة حساب احترافية وواضحة.';
        ...
        $lastDonationDate = !empty($stats['last_donation_at'])
            ? \Illuminate\Support\Carbon::parse($stats['last_donation_at'])->format('Y-m-d')
            : null;
    ...
    <div class="max-w-7xl mx-auto space-y-8">
        <section class="relative overflow-hidden rounded-[32px] border border-border bg-surface p-6 sm:p-8 lg:p-10">
            ...
            <div class="pointer-events-none absolute -bottom-20 -left-20 h-72 w-72 rounded-full blur-3xl opacity-20"
            ...
            <div class="flex flex-col xl:flex-row xl:items-start xl:justify-between gap-6">
                <div class="min-w-0 flex-1">
                    ...
                    <div class="mt-5 flex items-start gap-4">
                        ...
                        <div class="min-w-0">
                            <div class="text-sm text-subtext">
                                {{ $isEn ? 'Donor account' : 'حساب المتبرع' }}
                            </div>

                            <h1 class="mt-1 text-3xl sm:text-4xl font-black tracking-tight text-text">
                                {{ $donorName }}
                            </h1>

                            @if ($donorEmail)
                                <div class="mt-2 text-sm text-subtext break-all">
                                    {{ $donorEmail }}
                                </div>
                            @endif
                        </div>
                    ...
                    <p class="mt-5 text-subtext leading-relaxed max-w-3xl">
                        {{ $subtitle }}
                    </p>
                ...
                <div class="xl:w-[360px] shrink-0">
                    <div class="card-muted p-5 sm:p-6 space-y-4">
                        <div class="flex items-center justify-between gap-3">
                            <div>
                                <div class="text-sm text-subtext">
                                    {{ $isEn ? 'Quick overview' : 'نظرة سريعة' }}
                                </div>
                                <div class="mt-1 text-xl font-black text-text">
                                    {{ $isEn ? 'Your account at a glance' : 'حسابك بنظرة واحدة' }}
                                </div>
                            </div>
                        ...
                        <div class="grid grid-cols-2 gap-3 text-sm">
                            <div class="rounded-2xl border border-border bg-surface p-4">
                                <div class="text-subtext">{{ $isEn ? 'Paid' : 'مدفوع' }}</div>
                                <div class="mt-1 text-xl font-black text-text">{{ $statusCount('paid') }}</div>
                            </div>

                            <div class="rounded-2xl border border-border bg-surface p-4">
                                <div class="text-subtext">{{ $isEn ? 'Pending' : 'قيد الانتظار' }}</div>
                                <div class="mt-1 text-xl font-black text-text">{{ $statusCount('pending') }}</div>
                            </div>
                        </div>

                        @if ($lastDonationDate)
                            <div class="rounded-2xl border border-border bg-surface p-4 text-sm">
                                <div class="text-subtext">{{ $isEn ? 'Last donation' : 'آخر تبرع' }}</div>
                                <div class="mt-1 font-black text-text">{{ $lastDonationDate }}</div>
                            </div>
                        @endif

                        <div class="grid grid-cols-1 gap-3">
                            <a class="btn btn-primary w-full justify-center" href="{{ $donateUrl }}">
                                {{ $isEn ? 'Donate now' : 'تبرّع الآن' }}
                                <span aria-hidden="true">→</span>
                            </a>

                            <a class="btn btn-secondary w-full justify-center" href="{{ $donationsUrl }}">
                                {{ $isEn ? 'View all donations' : 'عرض كل التبرعات' }}
                            </a>

                            <a class="btn btn-secondary w-full justify-center" href="{{ $profileUrl }}">
            ...
            <aside class="xl:col-span-1 space-y-4">
                ...
                <div class="card-muted p-5">
                    <div class="font-black text-text">
                        {{ $isEn ? 'Trust note' : 'ملاحظة ثقة' }}
                    </div>
                    <div class="mt-2 text-sm text-subtext leading-relaxed">
                        {{ $isEn
                            ? 'Your account keeps your donation history, receipts, and status tracking in one secure place.'
                            : 'حسابك يجمع سجل التبرعات والإيصالات وتتبع الحالات في مكان واحد وآمن.' }}
                    </div>
                </div>
            </aside>
            ...
            <div class="xl:col-span-3 space-y-6">
                <section class="grid grid-cols-1 md:grid-cols-2 2xl:grid-cols-4 gap-5">
                    <div class="card p-6">
                        <div class="text-sm text-subtext font-semibold">
                            {{ $isEn ? 'Total donations' : 'عدد التبرعات' }}
                        </div>
                        <div class="mt-3 text-3xl font-black text-text">
                            {{ (int) ($stats['total_donations'] ?? 0) }}
                        </div>
                    </div>

                    <div class="card p-6">
                        <div class="text-sm text-subtext font-semibold">
                            {{ $isEn ? 'Paid donations' : 'التبرعات المدفوعة' }}
                        </div>
                        <div class="mt-3 text-3xl font-black text-text">
                            {{ (int) ($stats['paid_donations'] ?? 0) }}
                        </div>
                    </div>

                    <div class="card p-6">
                        <div class="text-sm text-subtext font-semibold">
                            {{ $isEn ? 'Total amount' : 'إجمالي المبلغ' }}
                        </div>
                        <div class="mt-3 text-3xl font-black text-text">
                            {{ $fmtMoney($stats['total_amount'] ?? 0) }}
                        </div>
                    </div>

                    <div class="card p-6">
                        <div class="text-sm text-subtext font-semibold">
                            {{ $isEn ? 'Campaigns supported' : 'الحملات المدعومة' }}
                        </div>
                        <div class="mt-3 text-3xl font-black text-text">
                            {{ (int) ($stats['campaigns_supported'] ?? 0) }}
                        </div>
                    </div>
                </section>
                ...
                <section class="grid grid-cols-1 2xl:grid-cols-3 gap-6">
                    <div class="2xl:col-span-2 card p-6 sm:p-7">
                        <div class="flex items-center justify-between gap-4 mb-5">
                            <div>
                                <h2 class="text-xl font-black text-text">
                                    {{ $isEn ? 'Donation status summary' : 'ملخص حالات التبرعات' }}
                                </h2>
                                <p class="mt-1 text-sm text-subtext">
                                    {{ $isEn ? 'A quick overview of your donation activity.' : 'نظرة سريعة على نشاط تبرعاتك.' }}
                                </p>
                            </div>
                            <a class="btn btn-secondary" href="{{ $donationsUrl }}">
                                {{ $isEn ? 'Open donations page' : 'فتح صفحة التبرعات' }}
                            </a>
                        </div>
                        ...
                        <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                            <div class="rounded-2xl border border-border bg-muted p-5">
                                ...
                                <div class="mt-2 text-2xl font-black text-text">{{ $statusCount('paid') }}</div>
                            </div>

                            <div class="rounded-2xl border border-border bg-muted p-5">
                                <div class="text-sm text-subtext">{{ $isEn ? 'Pending' : 'قيد الانتظار' }}</div>
                                <div class="mt-2 text-2xl font-black text-text">{{ $statusCount('pending') }}</div>
                            </div>

                            <div class="rounded-2xl border border-border bg-muted p-5">
                                <div class="text-sm text-subtext">{{ $isEn ? 'Failed' : 'فشل' }}</div>
                                <div class="mt-2 text-2xl font-black text-text">{{ $statusCount('failed') }}</div>
                            </div>

                            <div class="rounded-2xl border border-border bg-muted p-5">
                                <div class="text-sm text-subtext">{{ $isEn ? 'Refunded' : 'مسترد' }}</div>
                                <div class="mt-2 text-2xl font-black text-text">{{ $statusCount('refunded') }}</div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="card p-6 sm:p-7">
                        <div class="text-sm text-subtext">
                            {{ $isEn ? 'Latest successful donation' : 'آخر تبرع ناجح' }}
                        </div>
                        ...
                        @if ($latestPaid)
                            <div class="mt-4 rounded-3xl border border-border bg-muted p-5">
                                ...
                                <div class="mt-1 text-lg font-black text-text">
                                    {{ $campaignTitle($latestPaid->campaign) }}
                                </div>

                                <div class="mt-4 text-sm text-subtext">
                                    {{ $isEn ? 'Amount' : 'المبلغ' }}
                                </div>
                                <div class="mt-1 text-2xl font-black text-text">
                                    {{ $fmtMoney($latestPaid->amount, $latestPaid->currency) }}
                                </div>

                                <div class="mt-4 text-sm text-subtext">
                                    {{ $isEn ? 'Date' : 'التاريخ' }}
                                </div>
                                <div class="mt-1 font-semibold text-text">
                                    {{ optional($latestPaid->paid_at ?? $latestPaid->created_at)->format('Y-m-d') }}
                                </div>

                                @if ($latestPaid->receipt)
                                    <a class="mt-5 btn btn-secondary w-full justify-center"
                                        href="{{ locale_route('receipt.verify', ['receipt' => $latestPaid->receipt]) }}">
                                        {{ $isEn ? 'Open receipt' : 'فتح الإيصال' }}
                                    </a>
                                @endif
                            </div>
                        @else
                            <div
                                class="mt-4 rounded-3xl border border-border bg-muted p-5 text-sm text-subtext leading-relaxed">
                                {{ $isEn
                                    ? 'No successful donations yet. Once you complete a donation, the latest successful transaction will appear here.'
                                    : 'لا توجد تبرعات ناجحة بعد. عند إتمام أول تبرع ناجح سيظهر هنا آخر تبرع مكتمل.' }}
                            </div>

                            <a class="mt-4 btn btn-primary w-full justify-center" href="{{ $donateUrl }}">
                                {{ $isEn ? 'Make your first donation' : 'ابدأ أول تبرع' }}
                            </a>
                        @endif
                    </div>
                </section>
                ...
                <section class="card p-6 sm:p-7">
                    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-5">
                        <div>
                            <h2 class="text-xl font-black text-text">
                                {{ $isEn ? 'Recent donations' : 'أحدث التبرعات' }}
                            </h2>
                            <p class="mt-1 text-sm text-subtext">
                                {{ $isEn ? 'Your most recent activity with quick access to receipts.' : 'أحدث نشاطاتك مع وصول سريع إلى الإيصالات.' }}
                            </p>
                        </div>
                                    ...
                                    @foreach ($donations as $donation)
                                        ...
                                        <tr class="border-b last:border-0">
                                            ...
                                            <td class="py-4 text-center">
                                                <span
                                                    class="inline-flex items-center px-2.5 py-1 rounded-full border text-xs font-semibold {{ $statusClass }}">
                                                    {{ $statusText }}
                                                </span>
                                            </td>
                    ...
                    @else
                        <div class="rounded-3xl border border-border bg-muted p-10 text-center">
                            ...
                            <div class="mt-2 text-sm text-subtext">
                                {{ $isEn ? 'Start supporting campaigns and your activity will appear here.' : 'ابدأ بدعم الحملات وستظهر نشاطاتك هنا بشكل منظم.' }}
                            </div>
                            <div class="mt-5">
