...
@section('content')
    @php
        ...
        $subtitle = $isEn ? 'Monthly, annual, and campaign-related public documents.' : 'تقارير شهرية وسنوية ووثائق عامة مرتبطة بالحملات.';
    ...
    <section class="section-shell p-7 sm:p-10 mb-8">
        <div class="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4">
            <div>
                ...
                <div class="eyebrow mt-4">{{ $isEn ? 'Published documents' : 'وثائق منشورة' }}</div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
            @foreach ($reports as $report)
                <a href="{{ $reportShowUrl($report) }}" class="card p-6 hover:shadow-sm transition">
                    ...
                    <div class="mt-2 font-black text-lg text-text line-clamp-2">{{ $reportTitle($report) }}</div>
                    <div class="text-sm text-subtext mt-2 line-clamp-3">{{ $reportSummary($report) ?: ($isEn ? 'Open to read details and access the PDF.' : 'افتح لقراءة التفاصيل والوصول إلى ملف PDF.') }}</div>
    ...
    @else
        <div class="rounded-[28px] border border-border bg-muted p-10 text-center">
            <div class="text-lg font-black text-text">{{ $isEn ? 'No reports yet.' : 'لا توجد تقارير بعد.' }}</div>
            <div class="mt-2 text-sm text-subtext">{{ $isEn ? 'Please check back soon.' : 'يرجى العودة لاحقًا.' }}</div>
        </div>
    @endif
@endsection
